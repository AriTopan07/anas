...
@section('content')
        ...
        <section class="section">
            <div class="card">
                <div class="card-header">
                    ...
                    @if (NavHelper::cekAkses(Auth::user()->id, 'Units', 'add') == true)
                        ...
                        <tbody>
                            @foreach ($data['units'] as $item)
                                <tr>
                                    ...
                                    <td>
                                        @if (NavHelper::cekAkses(Auth::user()->id, 'Units', 'edit') == true)
                                        ...
                                        @endif
                                        @if (NavHelper::cekAkses(Auth::user()->id, 'Units', 'delete') == true)
    ...
    <div class="modal fade text-left" id="modal_add" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">Create Units</h4>
                    <button type="button" class="close btn-tutup" data-bs-dismiss="modal" aria-label="Close">
                        <i data-feather="x"></i>
                    </button>
                </div>
                ...
                @endif
                <form action="{{ route('units.create') }}" id="groupForm" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <label>Units Name</label>
                            <div class="form-group">
                                <input type="text" class="form-control" name="units_name" id="units_name" required
                                    value="{{ old('units_name') }}">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            $('#edit_nama').val(response.data.units_name);
                        }
                    }
                });
            });
